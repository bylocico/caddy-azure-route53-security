...
if (!latest) throw new Error('No stable Caddy versions found on Docker Hub')
...
if (!versions.some((v) => v.version === target.version)) {
	throw new Error(`caddy:${target.version} was not found on Docker Hub`)
}

if (target.version === current) {
	console.log(`Already targeting Caddy@${current}`)
	process.exit(0)
}
...
console.log(`Updated Caddy target ${current} -> ${target.version}`)
